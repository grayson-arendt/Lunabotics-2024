 ...
 * and Jerk.  Users can optionally provide a duty cycle feedforward.  This
 ...
 */
class DynamicMotionMagicDutyCycle : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DynamicMotionMagicDutyCycle *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        optionally provide a duty cycle feedforward.  This control requires
     *        use of a CANivore.
     * 
     ...
     *          duty cycle based, so relevant closed-loop gains will use fractional
     *          duty cycle for the numerator:  +1.0 represents full forward output.
     * 
    ...
    {
        std::stringstream ss;
        ss << "class: DynamicMotionMagicDutyCycle" << std::endl;
