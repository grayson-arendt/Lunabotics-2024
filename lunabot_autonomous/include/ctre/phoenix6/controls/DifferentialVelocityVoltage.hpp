 ...
 */
class DifferentialVelocityVoltage : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialVelocityVoltage *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialVelocityVoltage>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialVelocityVoltage(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, TargetVelocity.to<double>(), DifferentialPosition.to<double>(), EnableFOC, TargetSlot, DifferentialSlot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialVelocityVoltage" << std::endl;
