 ...
 */
class MotionMagicVelocityTorqueCurrentFOC : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<MotionMagicVelocityTorqueCurrentFOC *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<MotionMagicVelocityTorqueCurrentFOC>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlMotionMagicVelocityTorqueCurrentFOC(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, Velocity.to<double>(), Acceleration.to<double>(), EnableFOC, FeedForward.to<double>(), Slot, OverrideCoastDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
    ...
    {
        std::stringstream ss;
        ss << "class: MotionMagicVelocityTorqueCurrentFOC" << std::endl;
