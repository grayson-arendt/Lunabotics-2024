 ...
 */
class DifferentialStrictFollower : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialStrictFollower *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialStrictFollower>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialStrictFollower(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, MasterID);
    }
...
public:
     ...
     */
    DifferentialStrictFollower(int MasterID) : ControlRequest{"DifferentialStrictFollower"},
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialStrictFollower" << std::endl;
