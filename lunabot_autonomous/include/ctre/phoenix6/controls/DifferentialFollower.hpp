 ...
 */
class DifferentialFollower : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialFollower *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialFollower>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialFollower(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, MasterID, OpposeMasterDirection);
    }
...
public:
     ...
     * 
     * \param MasterID    Device ID of the differential master to follow.
     ...
     *                                 mechanically spin in opposite directions.
     */
    DifferentialFollower(int MasterID, bool OpposeMasterDirection) : ControlRequest{"DifferentialFollower"},
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialFollower" << std::endl;
