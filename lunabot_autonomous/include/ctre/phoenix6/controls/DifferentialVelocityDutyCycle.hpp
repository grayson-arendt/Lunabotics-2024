 ...
 */
class DifferentialVelocityDutyCycle : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialVelocityDutyCycle *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialVelocityDutyCycle>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialVelocityDutyCycle(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, TargetVelocity.to<double>(), DifferentialPosition.to<double>(), EnableFOC, TargetSlot, DifferentialSlot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
    ...
    /**
     * \brief Request PID to target velocity with a differential position setpoint.
     * 
     ...
     * 
     * \param TargetVelocity    Average velocity to drive toward in rotations per
     *                          second.
    ...
    DifferentialVelocityDutyCycle(units::angular_velocity::turns_per_second_t TargetVelocity, units::angle::turn_t DifferentialPosition, bool EnableFOC = true, int TargetSlot = 0, int DifferentialSlot = 1, bool OverrideBrakeDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"DifferentialVelocityDutyCycle"},
        TargetVelocity{std::move(TargetVelocity)},
    ...
    /**
     * \brief Modifies this Control Request's TargetVelocity parameter and returns itself for
     *        method-chaining and easier to use request API.
     * \param newTargetVelocity Parameter to modify
     * \returns Itself
     */
    DifferentialVelocityDutyCycle& WithTargetVelocity(units::angular_velocity::turns_per_second_t newTargetVelocity)
    {
        TargetVelocity = std::move(newTargetVelocity);
        return *this;
    }
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialVelocityDutyCycle" << std::endl;
        ss << "TargetVelocity: " << TargetVelocity.to<double>() << std::endl;
    ...
    std::map<std::string, std::string> GetControlInfo() const override
    {
        ...
        ss << TargetVelocity.to<double>(); controlInfo["TargetVelocity"] = ss.str(); ss.str(std::string{});
