 ...
 */
class DifferentialPositionVoltage : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialPositionVoltage *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialPositionVoltage>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialPositionVoltage(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, TargetPosition.to<double>(), DifferentialPosition.to<double>(), EnableFOC, TargetSlot, DifferentialSlot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
     ...
     *                              power.
     */
    DifferentialPositionVoltage(units::angle::turn_t TargetPosition, units::angle::turn_t DifferentialPosition, bool EnableFOC = true, int TargetSlot = 0, int DifferentialSlot = 1, bool OverrideBrakeDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"DifferentialPositionVoltage"},
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialPositionVoltage" << std::endl;
