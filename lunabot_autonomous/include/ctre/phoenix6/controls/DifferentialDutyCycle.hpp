 ...
 */
class DifferentialDutyCycle : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialDutyCycle *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialDutyCycle>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialDutyCycle(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, TargetOutput.to<double>(), DifferentialPosition.to<double>(), EnableFOC, DifferentialSlot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
    ...
    /**
     * \brief Request a specified motor duty cycle with a differential position
     *        closed-loop.
     * 
     ...
     * 
     * \param TargetOutput    Proportion of supply voltage to apply in fractional
     ...
     *                              power.
     */
    DifferentialDutyCycle(units::dimensionless::scalar_t TargetOutput, units::angle::turn_t DifferentialPosition, bool EnableFOC = true, int DifferentialSlot = 1, bool OverrideBrakeDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"DifferentialDutyCycle"},
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialDutyCycle" << std::endl;
