 ...
 * Motion Magic® will do its best to adjust the profile.  This control mode is voltage-based, so relevant
 * closed-loop gains will use Volts for the numerator.
 */
class MotionMagicExpoVoltage : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<MotionMagicExpoVoltage *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<MotionMagicExpoVoltage>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlMotionMagicExpoVoltage(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, Position.to<double>(), EnableFOC, FeedForward.to<double>(), Slot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
     ...
     *                              power.
     */
    MotionMagicExpoVoltage(units::angle::turn_t Position, bool EnableFOC = true, units::voltage::volt_t FeedForward = 0.0_V, int Slot = 0, bool OverrideBrakeDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"MotionMagicExpoVoltage"},
    ...
    {
        std::stringstream ss;
        ss << "class: MotionMagicExpoVoltage" << std::endl;
