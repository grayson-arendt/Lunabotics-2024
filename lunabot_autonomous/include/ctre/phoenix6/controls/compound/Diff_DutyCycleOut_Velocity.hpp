 ...
 * target.
 */
class Diff_DutyCycleOut_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_DutyCycleOut_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<Diff_DutyCycleOut_Velocity>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDiff_DutyCycleOut_Velocity(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, AverageRequest.Output.to<double>(), AverageRequest.EnableFOC, AverageRequest.OverrideBrakeDurNeutral, AverageRequest.LimitForwardMotion, AverageRequest.LimitReverseMotion, DifferentialRequest.Velocity.to<double>(), DifferentialRequest.Acceleration.to<double>(), DifferentialRequest.EnableFOC, DifferentialRequest.FeedForward.to<double>(), DifferentialRequest.Slot, DifferentialRequest.OverrideBrakeDurNeutral, DifferentialRequest.LimitForwardMotion, DifferentialRequest.LimitReverseMotion);
    }
...
public:
     ...
     *        Differential control with duty
     *        cycle average target and velocity difference target.
     * 
     ...
     *                               mechanism.
     */
    Diff_DutyCycleOut_Velocity(DutyCycleOut AverageRequest, VelocityDutyCycle DifferentialRequest) : ControlRequest{"Diff_DutyCycleOut_Velocity"},
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Velocity& WithAverageRequest(DutyCycleOut newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Velocity& WithDifferentialRequest(VelocityDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_DutyCycleOut_Velocity" << std::endl;
