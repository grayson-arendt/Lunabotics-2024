 ...
 */
class Diff_MotionMagicVoltage_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_MotionMagicVoltage_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     */
    MotionMagicVoltage AverageRequest;
    /**
     ...
     *        Magic® average target and position difference target using voltage
     *        control.
     * 
     ...
     *                               mechanism.
     */
    Diff_MotionMagicVoltage_Position(MotionMagicVoltage AverageRequest, PositionVoltage DifferentialRequest) : ControlRequest{"Diff_MotionMagicVoltage_Position"},
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Position& WithAverageRequest(MotionMagicVoltage newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Position& WithDifferentialRequest(PositionVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_MotionMagicVoltage_Position" << std::endl;
