 ...
 * target.
 */
class Diff_VoltageOut_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VoltageOut_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<Diff_VoltageOut_Velocity>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDiff_VoltageOut_Velocity(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, AverageRequest.Output.to<double>(), AverageRequest.EnableFOC, AverageRequest.OverrideBrakeDurNeutral, AverageRequest.LimitForwardMotion, AverageRequest.LimitReverseMotion, DifferentialRequest.Velocity.to<double>(), DifferentialRequest.Acceleration.to<double>(), DifferentialRequest.EnableFOC, DifferentialRequest.FeedForward.to<double>(), DifferentialRequest.Slot, DifferentialRequest.OverrideBrakeDurNeutral, DifferentialRequest.LimitForwardMotion, DifferentialRequest.LimitReverseMotion);
    }
...
public:
     ...
     *        Differential control with voltage
     *        average target and velocity difference target.
     * 
     ...
     *                               mechanism.
     */
    Diff_VoltageOut_Velocity(VoltageOut AverageRequest, VelocityVoltage DifferentialRequest) : ControlRequest{"Diff_VoltageOut_Velocity"},
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Velocity& WithAverageRequest(VoltageOut newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Velocity& WithDifferentialRequest(VelocityVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VoltageOut_Velocity" << std::endl;
