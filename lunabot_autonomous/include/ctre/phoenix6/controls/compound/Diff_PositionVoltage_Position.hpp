 ...
 */
class Diff_PositionVoltage_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_PositionVoltage_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               mechanism.
     */
    Diff_PositionVoltage_Position(PositionVoltage AverageRequest, PositionVoltage DifferentialRequest) : ControlRequest{"Diff_PositionVoltage_Position"},
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Position& WithAverageRequest(PositionVoltage newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Position& WithDifferentialRequest(PositionVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_PositionVoltage_Position" << std::endl;
