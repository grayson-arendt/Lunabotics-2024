 ...
 */
class Diff_PositionTorqueCurrentFOC_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_PositionTorqueCurrentFOC_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               of the mechanism.
     */
    Diff_PositionTorqueCurrentFOC_Position(PositionTorqueCurrentFOC AverageRequest, PositionTorqueCurrentFOC DifferentialRequest) : ControlRequest{"Diff_PositionTorqueCurrentFOC_Position"},
     ...
     * \returns Itself
     */
    Diff_PositionTorqueCurrentFOC_Position& WithAverageRequest(PositionTorqueCurrentFOC newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionTorqueCurrentFOC_Position& WithDifferentialRequest(PositionTorqueCurrentFOC newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionTorqueCurrentFOC_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_PositionTorqueCurrentFOC_Position" << std::endl;
