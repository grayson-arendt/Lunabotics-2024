 ...
 */
class Diff_VelocityDutyCycle_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VelocityDutyCycle_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               mechanism.
     */
    Diff_VelocityDutyCycle_Position(VelocityDutyCycle AverageRequest, PositionDutyCycle DifferentialRequest) : ControlRequest{"Diff_VelocityDutyCycle_Position"},
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Position& WithAverageRequest(VelocityDutyCycle newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Position& WithDifferentialRequest(PositionDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VelocityDutyCycle_Position" << std::endl;
