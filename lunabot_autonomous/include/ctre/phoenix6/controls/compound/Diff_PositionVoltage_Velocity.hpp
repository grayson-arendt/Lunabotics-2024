 ...
 */
class Diff_PositionVoltage_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_PositionVoltage_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     */
    PositionVoltage AverageRequest;
    /**
     ...
     *        average target and velocity difference target using voltage control.
     * 
     ...
     *                               mechanism.
     */
    Diff_PositionVoltage_Velocity(PositionVoltage AverageRequest, VelocityVoltage DifferentialRequest) : ControlRequest{"Diff_PositionVoltage_Velocity"},
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Velocity& WithAverageRequest(PositionVoltage newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Velocity& WithDifferentialRequest(VelocityVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionVoltage_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_PositionVoltage_Velocity" << std::endl;
