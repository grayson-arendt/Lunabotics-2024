 ...
 * difference target.
 */
class Diff_TorqueCurrentFOC_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_TorqueCurrentFOC_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<Diff_TorqueCurrentFOC_Velocity>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDiff_TorqueCurrentFOC_Velocity(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, AverageRequest.Output.to<double>(), AverageRequest.MaxAbsDutyCycle.to<double>(), AverageRequest.Deadband.to<double>(), AverageRequest.OverrideCoastDurNeutral, AverageRequest.LimitForwardMotion, AverageRequest.LimitReverseMotion, DifferentialRequest.Velocity.to<double>(), DifferentialRequest.Acceleration.to<double>(), DifferentialRequest.FeedForward.to<double>(), DifferentialRequest.Slot, DifferentialRequest.OverrideCoastDurNeutral, DifferentialRequest.LimitForwardMotion, DifferentialRequest.LimitReverseMotion);
    }
...
public:
     ...
     *        Differential control with torque
     *        current average target and velocity difference target.
     * 
     ...
     *                               of the mechanism.
     */
    Diff_TorqueCurrentFOC_Velocity(TorqueCurrentFOC AverageRequest, VelocityTorqueCurrentFOC DifferentialRequest) : ControlRequest{"Diff_TorqueCurrentFOC_Velocity"},
     ...
     * \returns Itself
     */
    Diff_TorqueCurrentFOC_Velocity& WithAverageRequest(TorqueCurrentFOC newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_TorqueCurrentFOC_Velocity& WithDifferentialRequest(VelocityTorqueCurrentFOC newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_TorqueCurrentFOC_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_TorqueCurrentFOC_Velocity" << std::endl;
        ss << "AverageRequest:" << std::endl;
        ss << "    Output: " << AverageRequest.Output.to<double>() << std::endl;
        ss << "    MaxAbsDutyCycle: " << AverageRequest.MaxAbsDutyCycle.to<double>() << std::endl;
        ss << "    Deadband: " << AverageRequest.Deadband.to<double>() << std::endl;
