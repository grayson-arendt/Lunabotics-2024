 ...
 */
class Diff_PositionDutyCycle_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_PositionDutyCycle_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        average target and velocity difference target using dutycycle control.
     * 
     ...
     *                               mechanism.
     */
    Diff_PositionDutyCycle_Velocity(PositionDutyCycle AverageRequest, VelocityDutyCycle DifferentialRequest) : ControlRequest{"Diff_PositionDutyCycle_Velocity"},
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Velocity& WithAverageRequest(PositionDutyCycle newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Velocity& WithDifferentialRequest(VelocityDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_PositionDutyCycle_Velocity" << std::endl;
