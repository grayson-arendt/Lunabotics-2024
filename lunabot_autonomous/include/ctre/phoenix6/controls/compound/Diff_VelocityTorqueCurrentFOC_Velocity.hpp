 ...
 */
class Diff_VelocityTorqueCurrentFOC_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VelocityTorqueCurrentFOC_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               of the mechanism.
     */
    Diff_VelocityTorqueCurrentFOC_Velocity(VelocityTorqueCurrentFOC AverageRequest, VelocityTorqueCurrentFOC DifferentialRequest) : ControlRequest{"Diff_VelocityTorqueCurrentFOC_Velocity"},
     ...
     * \returns Itself
     */
    Diff_VelocityTorqueCurrentFOC_Velocity& WithAverageRequest(VelocityTorqueCurrentFOC newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityTorqueCurrentFOC_Velocity& WithDifferentialRequest(VelocityTorqueCurrentFOC newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityTorqueCurrentFOC_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VelocityTorqueCurrentFOC_Velocity" << std::endl;
