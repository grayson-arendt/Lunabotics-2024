 ...
 * target.
 */
class Diff_VoltageOut_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VoltageOut_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        average target and position difference target.
     * 
     ...
     *                               mechanism.
     */
    Diff_VoltageOut_Position(VoltageOut AverageRequest, PositionVoltage DifferentialRequest) : ControlRequest{"Diff_VoltageOut_Position"},
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Position& WithAverageRequest(VoltageOut newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Position& WithDifferentialRequest(PositionVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VoltageOut_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VoltageOut_Position" << std::endl;
