 ...
 */
class Diff_MotionMagicDutyCycle_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_MotionMagicDutyCycle_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        Magic® average target and position difference target using dutycycle
     *        control.
     * 
     ...
     *                               mechanism.
     */
    Diff_MotionMagicDutyCycle_Position(MotionMagicDutyCycle AverageRequest, PositionDutyCycle DifferentialRequest) : ControlRequest{"Diff_MotionMagicDutyCycle_Position"},
     ...
     * \returns Itself
     */
    Diff_MotionMagicDutyCycle_Position& WithAverageRequest(MotionMagicDutyCycle newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicDutyCycle_Position& WithDifferentialRequest(PositionDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicDutyCycle_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_MotionMagicDutyCycle_Position" << std::endl;
