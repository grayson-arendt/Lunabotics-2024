 ...
 */
class Diff_MotionMagicVoltage_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_MotionMagicVoltage_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        Magic® average target and velocity difference target using voltage
     *        control.
     * 
     ...
     *                               mechanism.
     */
    Diff_MotionMagicVoltage_Velocity(MotionMagicVoltage AverageRequest, VelocityVoltage DifferentialRequest) : ControlRequest{"Diff_MotionMagicVoltage_Velocity"},
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Velocity& WithAverageRequest(MotionMagicVoltage newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Velocity& WithDifferentialRequest(VelocityVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicVoltage_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_MotionMagicVoltage_Velocity" << std::endl;
