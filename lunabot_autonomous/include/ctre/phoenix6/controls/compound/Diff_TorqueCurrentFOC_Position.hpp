...
class Diff_TorqueCurrentFOC_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        ...
        return c_ctre_phoenix6_RequestControlDiff_TorqueCurrentFOC_Position(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, AverageRequest.Output.to<double>(), AverageRequest.MaxAbsDutyCycle.to<double>(), AverageRequest.Deadband.to<double>(), AverageRequest.OverrideCoastDurNeutral, AverageRequest.LimitForwardMotion, AverageRequest.LimitReverseMotion, DifferentialRequest.Position.to<double>(), DifferentialRequest.Velocity.to<double>(), DifferentialRequest.FeedForward.to<double>(), DifferentialRequest.Slot, DifferentialRequest.OverrideCoastDurNeutral, DifferentialRequest.LimitForwardMotion, DifferentialRequest.LimitReverseMotion);
    }
...
public:
     ...
     *        current average target and position difference target.
     * 
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_TorqueCurrentFOC_Position" << std::endl;
