 ...
 */
class Diff_VelocityVoltage_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VelocityVoltage_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               mechanism.
     */
    Diff_VelocityVoltage_Velocity(VelocityVoltage AverageRequest, VelocityVoltage DifferentialRequest) : ControlRequest{"Diff_VelocityVoltage_Velocity"},
     ...
     * \returns Itself
     */
    Diff_VelocityVoltage_Velocity& WithAverageRequest(VelocityVoltage newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityVoltage_Velocity& WithDifferentialRequest(VelocityVoltage newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityVoltage_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VelocityVoltage_Velocity" << std::endl;
