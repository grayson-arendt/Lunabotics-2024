 ...
 */
class Diff_VelocityDutyCycle_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_VelocityDutyCycle_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *                               mechanism.
     */
    Diff_VelocityDutyCycle_Velocity(VelocityDutyCycle AverageRequest, VelocityDutyCycle DifferentialRequest) : ControlRequest{"Diff_VelocityDutyCycle_Velocity"},
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Velocity& WithAverageRequest(VelocityDutyCycle newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Velocity& WithDifferentialRequest(VelocityDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_VelocityDutyCycle_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_VelocityDutyCycle_Velocity" << std::endl;
