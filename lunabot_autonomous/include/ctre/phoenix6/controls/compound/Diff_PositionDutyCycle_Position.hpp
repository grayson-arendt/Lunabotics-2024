 ...
 */
class Diff_PositionDutyCycle_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_PositionDutyCycle_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        average target and position difference target using dutycycle control.
     * 
     ...
     *                               mechanism.
     */
    Diff_PositionDutyCycle_Position(PositionDutyCycle AverageRequest, PositionDutyCycle DifferentialRequest) : ControlRequest{"Diff_PositionDutyCycle_Position"},
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Position& WithAverageRequest(PositionDutyCycle newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Position& WithDifferentialRequest(PositionDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_PositionDutyCycle_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_PositionDutyCycle_Position" << std::endl;
