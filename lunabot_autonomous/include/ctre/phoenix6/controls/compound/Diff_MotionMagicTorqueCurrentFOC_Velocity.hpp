 ...
 */
class Diff_MotionMagicTorqueCurrentFOC_Velocity : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_MotionMagicTorqueCurrentFOC_Velocity *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
...
public:
     ...
     *        Magic® average target and velocity difference target using torque
     *        current control.
     * 
     ...
     *                               of the mechanism.
     */
    Diff_MotionMagicTorqueCurrentFOC_Velocity(MotionMagicTorqueCurrentFOC AverageRequest, VelocityTorqueCurrentFOC DifferentialRequest) : ControlRequest{"Diff_MotionMagicTorqueCurrentFOC_Velocity"},
     ...
     * \returns Itself
     */
    Diff_MotionMagicTorqueCurrentFOC_Velocity& WithAverageRequest(MotionMagicTorqueCurrentFOC newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicTorqueCurrentFOC_Velocity& WithDifferentialRequest(VelocityTorqueCurrentFOC newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_MotionMagicTorqueCurrentFOC_Velocity &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_MotionMagicTorqueCurrentFOC_Velocity" << std::endl;
