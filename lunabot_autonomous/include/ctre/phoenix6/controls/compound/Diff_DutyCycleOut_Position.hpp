 ...
 * target.
 */
class Diff_DutyCycleOut_Position : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<Diff_DutyCycleOut_Position *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<Diff_DutyCycleOut_Position>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDiff_DutyCycleOut_Position(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, AverageRequest.Output.to<double>(), AverageRequest.EnableFOC, AverageRequest.OverrideBrakeDurNeutral, AverageRequest.LimitForwardMotion, AverageRequest.LimitReverseMotion, DifferentialRequest.Position.to<double>(), DifferentialRequest.Velocity.to<double>(), DifferentialRequest.EnableFOC, DifferentialRequest.FeedForward.to<double>(), DifferentialRequest.Slot, DifferentialRequest.OverrideBrakeDurNeutral, DifferentialRequest.LimitForwardMotion, DifferentialRequest.LimitReverseMotion);
    }
...
public:
     ...
     *        cycle average target and position difference target.
     * 
     ...
     *                               mechanism.
     */
    Diff_DutyCycleOut_Position(DutyCycleOut AverageRequest, PositionDutyCycle DifferentialRequest) : ControlRequest{"Diff_DutyCycleOut_Position"},
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Position& WithAverageRequest(DutyCycleOut newAverageRequest)
    {
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Position& WithDifferentialRequest(PositionDutyCycle newDifferentialRequest)
    {
     ...
     * \returns Itself
     */
    Diff_DutyCycleOut_Position &WithUpdateFreqHz(units::frequency::hertz_t newUpdateFreqHz)
    {
    ...
    {
        std::stringstream ss;
        ss << "class: Diff_DutyCycleOut_Position" << std::endl;
