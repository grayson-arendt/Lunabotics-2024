 ...
 * Motion Magic® will do its best to adjust the profile.  This control mode is based on torque current, so
 * relevant closed-loop gains will use Amperes for the numerator.
 */
class MotionMagicExpoTorqueCurrentFOC : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<MotionMagicExpoTorqueCurrentFOC *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<MotionMagicExpoTorqueCurrentFOC>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlMotionMagicExpoTorqueCurrentFOC(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, Position.to<double>(), FeedForward.to<double>(), Slot, OverrideCoastDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
     ...
     *        position using an exponential motion profile.  Users can optionally
     ...
     *                              power.
     */
    MotionMagicExpoTorqueCurrentFOC(units::angle::turn_t Position, units::current::ampere_t FeedForward = 0.0_A, int Slot = 0, bool OverrideCoastDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"MotionMagicExpoTorqueCurrentFOC"},
    ...
    {
        std::stringstream ss;
        ss << "class: MotionMagicExpoTorqueCurrentFOC" << std::endl;
