 ...
 */
class DifferentialMotionMagicVoltage : public ControlRequest
{
    ctre::phoenix::StatusCode SendRequest(const char *network, uint32_t deviceHash, bool cancelOtherRequests, std::shared_ptr<ControlRequest> &req) override
    {
        if (req.get() != this)
        {
            auto const reqCast = dynamic_cast<DifferentialMotionMagicVoltage *>(req.get());
            if (reqCast != nullptr)
            {
                *reqCast = *this;
            }
            ...
            {
                req = std::make_shared<DifferentialMotionMagicVoltage>(*this);
            }
        }

        return c_ctre_phoenix6_RequestControlDifferentialMotionMagicVoltage(network, deviceHash, UpdateFreqHz.to<double>(), cancelOtherRequests, TargetPosition.to<double>(), DifferentialPosition.to<double>(), EnableFOC, TargetSlot, DifferentialSlot, OverrideBrakeDurNeutral, LimitForwardMotion, LimitReverseMotion);
    }
...
public:
     ...
     *                              power.
     */
    DifferentialMotionMagicVoltage(units::angle::turn_t TargetPosition, units::angle::turn_t DifferentialPosition, bool EnableFOC = true, int TargetSlot = 0, int DifferentialSlot = 1, bool OverrideBrakeDurNeutral = false, bool LimitForwardMotion = false, bool LimitReverseMotion = false) : ControlRequest{"DifferentialMotionMagicVoltage"},
    ...
    {
        std::stringstream ss;
        ss << "class: DifferentialMotionMagicVoltage" << std::endl;
