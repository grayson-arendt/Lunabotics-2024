...
public:
    ...
    explicit Navigator(const rclcpp::NodeOptions & options) : Node("navigator_client", options) {
...
private:
    ...
    void feedback_callback(GoalHandleNavigate::SharedPtr, const std::shared_ptr<const NavigateToPose::Feedback> feedback) {
        ...
        // List navigation time and distance remaining
        RCLCPP_INFO(this->get_logger(), "Current pose: ", feedback->navigation_time);
        RCLCPP_INFO(this->get_logger(), "Distance remaining: ", feedback->distance_remaining);
    }
