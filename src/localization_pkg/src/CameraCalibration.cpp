...
/* This is sample code that I (Grayson Arendt) found and adjusted purely for OpenCV camera calibration.
 * This will give the cameraMatrix and distortionCoeffs needed for pose estimation.
 */

int main() {
    // Define the size of the checkerboard (inner corners).
    cv::Size boardSize(8, 6); // 9x7 checkerboard

    // Define the size of each square in millimeters.
    float squareSize = 30.0; // 30mm squares
    ...
    for (int i = 1; i <= numImages; ++i) {
        // Load the image.
        std::string imagePath = "/home/grayson/ros2_ws/src/src/localization_pkg/images/image" + std::to_string(i) + ".png";
