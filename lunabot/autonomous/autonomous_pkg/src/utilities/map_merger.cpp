...
class MapMerger : public rclcpp::Node
{
  public:
    ...
    MapMerger() : Node("occupancy_grid_merger_node")
    {
        ...
        merged_occupancy_grid_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("map", 10);
  ...
  private:
    ...
    {
        RCLCPP_INFO(get_logger(), "Received occupancy_grid1 message with timestamp %f",
    ...
    {
        RCLCPP_INFO(get_logger(), "Received occupancy_grid2 message with timestamp %f",
                    msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9);
        ...
        for (size_t i = 0; i < merged_occupancy_grid_.data.size(); ++i)
        {
            if (msg->data[i] != std::numeric_limits<int8_t>::min() &&
                msg->data[i] != std::numeric_limits<int8_t>::max())
            {
    ...
    void resizeMergedOccupancyGrid(unsigned int width, unsigned int height)
    {
        if (width > merged_occupancy_grid_.info.width || height > merged_occupancy_grid_.info.height)
        {
            ...
            merged_occupancy_grid_.data.resize(merged_occupancy_grid_.info.width * merged_occupancy_grid_.info.height,
                                               0);
        }
    }
